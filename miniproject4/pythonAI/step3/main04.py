...
def mosaic_region(img, x1, y1, x2, y2, blocks=16):
    """
    지정 영역을 픽셀화(모자이크) 처리
    - blocks: 모자이크 블록 개수(가로 기준). 값 클수록 거칠게(더 크게) 보임.
    """
    h, w = img.shape[:2]
    ...
    # 너무 작은 박스에서 0 나눗셈 방지
    blocks = max(4, blocks)
    # 블록 크기를 영역 너비에 맞춰 대략 blocks개로 보이게 축소
    ...
    for result in results:
        ...
        for box, confidence, class_id in zip(boxes, confidences, class_ids):
            ...
            if label.lower() == 'person':
                # 사람(person)은 사각형 대신 모자이크
                mosaic_region(image, x1, y1, x2, y2, blocks=18)
                # 필요하면 라벨만 살짝 위에 표시(선택)
                # cv2.putText(image, f'person {conf:.2f}', (x1, max(0, y1-6)),
                #             cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 3)
                # cv2.putText(image, f'person {conf:.2f}', (x1, max(0, y1-6)),
                #             cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            else:
                # 그 외 클래스는 기존처럼 박스/라벨 표시
                cv2.rectangle(image, (x1, y1), (x2, y2), colors[cid], 2)
                cv2.putText(image, f'{label} - {conf:.2f}', (x1, y1),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, colors[cid], 2)
